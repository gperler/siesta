...
class ProductUUID implements ArraySerializable
{

    const TABLE_NAME = "ProductUUID";
    ...
    {
        $spCall = ($this->_existing) ? "CALL ProductUUID_U(" : "CALL ProductUUID_I(";
    ...
    public function getRelatedProductList(bool $forceReload = false, string $connectionName = null) : array
    {
        if ($this->relatedProductList === null || $forceReload) {
            $this->relatedProductList = ProductUUIDService::getInstance()->getProductUUIDJoinProductRelatedUUID($this->id, $connectionName);
        }
     ...
     * @return void
     */
    public function addToRelatedProductList(ProductUUID $entity)
    {
        $mapping = ProductRelatedUUIDService::getInstance()->newInstance();
    ...
    {
        $connection = ConnectionFactory::getConnection($connectionName);
        $localProductUUIDId = Escaper::quoteInt($this->id);
        $foreignProductUUIDId = Escaper::quoteInt($id);
        $connection->execute("CALL ProductRelatedUUID_D_A_ProductUUID_relatedProduc($localProductUUIDId,$foreignProductUUIDId)");
        if ($id === null) {
     ...
     * @return void
     */
    public function deleteAssignedProductUUID(int $id = null, string $connectionName = null)
    {
        $connection = ConnectionFactory::getConnection($connectionName);
        $localProductUUIDId = Escaper::quoteInt($this->id);
        $foreignProductUUIDId = Escaper::quoteInt($id);
        $connection->execute("CALL ProductUUID_D_JOIN_ProductRelatedUUID_relatedPro($localProductUUIDId,$foreignProductUUIDId)");
        if ($id === null) {
            $this->relatedProductList = [];
            $this->relatedProductListMapping = [];
            return;
     ...
     * @return bool
     */
    public function arePrimaryKeyIdentical(ProductUUID $entity = null) : bool
    {
        if ($entity === null) {
            return false;
        }
