...
class CartItemUUIDService
{
     ...
     * @return void
     */
    public function deleteEntityByCartReference(string $cartId, string $id = null, string $connectionName = null)
    {
        $connection = ConnectionFactory::getConnection($connectionName);
        $cartId = Escaper::quoteString($connection, $cartId);
        $connection = ConnectionFactory::getConnection($connectionName);
        $id = Escaper::quoteString($connection, $id);
        $connection->execute("CALL CartItemUUID_DB_R_cart($cartId,$id)");
    }
