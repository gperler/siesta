<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="delete">
        <xsl:if test="/entity/@hasPrimaryKey = 'true'">
             ...
             * @return void
             */
            public function delete($connectionName=null)
            {
                $connection = ConnectionFactory::getConnection($connectionName);
                ...
                <xsl:for-each select="/entity/attribute[@primaryKey = 'true']">
                    <xsl:choose>
                        <xsl:when test="@type = 'bool' or @type = 'int' or @type = 'float'">
                            $<xsl:value-of select="@name"/> = Util::quoteNumber($this-><xsl:value-of select="@name"/>);
                        </xsl:when>
                        <xsl:when test="@type = 'string'">
                            $<xsl:value-of select="@name"/> = Util::quoteEscape($connection, $this-><xsl:value-of select="@name"/>);
                        </xsl:when>
                        <xsl:when test="@type = 'DateTime'">
                            <xsl:choose>
                                <xsl:when test="@dbType = 'DATETIME'">
                                    $<xsl:value-of select="@name"/> = Util::quoteDateTime($this-><xsl:value-of select="@name"/>);
                                </xsl:when>
                                <xsl:when test="@dbType='DATE'">
                                    $<xsl:value-of select="@name"/> =  Util::quoteDate($this-><xsl:value-of select="@name"/>);
                                </xsl:when>
                                <xsl:when test="@dbType='TIME'">
                                    $<xsl:value-of select="@name"/> = Util::quoteTime($this-><xsl:value-of select="@name"/>);
                                </xsl:when>
                            </xsl:choose>
                        </xsl:when>
                    </xsl:choose>
                ...
                $connection->execute("CALL <xsl:value-of select="/entity/standardStoredProcedures/@deleteByPrimaryKey"/>(<xsl:for-each select="/entity/attribute[@primaryKey = 'true']">$<xsl:value-of select="@name"/><xsl:if test="position() != last()">,</xsl:if></xsl:for-each>)");
            }
        </xsl:if>
    </xsl:template>
